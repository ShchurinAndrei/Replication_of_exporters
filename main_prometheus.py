...
linKey = r'/home/sigma.sbrf.ru@22682851/.ssh/rsa_key_prometheus_NT'
...
test_command = {
    ...
    'check_exporter': 'sudo -u prometheus bash -c "crontab -l | grep -q check_exporter && echo 1 || echo 0"'
}
...
def available_exporter(host, user, exporter):
    pp_res, s = km_lin.linCommand(f'test -d /opt/prometheus/{exporter}/ && echo 1 || echo 0', host, user, linKey)
    if s.strip()=='1':
        flag_exporter = True
...
# функция создание пользователя prometheus и копирования экспортеров
def replication_exporter(host, user, db, exporter):
    for file in exporter_files[exporter]:
        ...
        pp_res, s = km_lin.linCommand(f'mkdir /opt/prometheus/{exporter}', host, user, linKey)
        path_out = file_path + f'{exporter}/' + file
        path_in = f'/home/{user}/'
        pp_res_transfer = km_lin.linPutFile(host, user, linKey, path_in, path_out)
        pp_res, s = km_lin.linCommand(f'', host, user, linKey)
        pp_res, s = km_lin.linCommand(f'mv /home/{user}/{file} /opt/prometheus/{exporter}/', host, user, linKey)
        pp_res, s = km_lin.linCommand(f'mv /home/{user}/{file} /opt/prometheus/{exporter}/', host, user, linKey)
    pp_res, s = km_lin.linCommand('/usr/bin/chmod -R 755 /opt/prometheus', host, user, linKey)
...
def restarted_exporter(host, user, exporter):
    ...
    for pid in s.split():
        pp_res, s = km_lin.linCommand(f'sudo -u prometheus bash -c "kill -9 {pid}"', host, user, linKey)
    pp_res, s = km_lin.linCommand(start_command[exporter], host, user, linKey)
    pp_res, s = km_lin.linCommand(test_command[exporter], host, user, linKey)
    if s.strip()=='1':
...
for host, user, db in zip(phost, puser, pdb): # цикл по связкам хост-уз-бд
    print(host)
    create_command = [
        'sudo /usr/sbin/adduser -s /sbin/nologin -m prometheus',
        'sudo /usr/bin/loginctl enable-linger prometheus',
        'sudo /usr/bin/mkdir /opt/prometheus',
        f'sudo /usr/bin/chown -R {user}/:prometheus /opt/prometheus',
        'sudo /usr/bin/chmod -R 755 /opt/prometheus'
    ]

    # проверка наличия пользователя prometheus
    pp_res, s = km_lin.linCommand(f'test -d /opt/prometheus/ && echo 1 || echo 0', host, user, linKey)
    if s.strip()=='0':
        for command in create_command:
            pp_res, s = km_lin.linCommand(command, host, user, linKey)
            print('\tПользователя prometheus создан!')
    for exporter in pexporter:  # цикл по экспортерам
        start_command = {
            'node_exporter': 'sudo -u prometheus bash -c "nohup bash -c /opt/prometheus/node_exporter/node_exporter >/dev/null 2>&1 &" >/dev/null 2>&1 &',
            'postgres_exporter': f'sudo -u prometheus /opt/prometheus/postgres_exporter/env',
            'sql_exporter': f'sudo -u prometheus /opt/prometheus/sql_exporter/env_s',
            'check_exporter': 'sudo -u prometheus bash -c "(cronatab -l 2>/dev/null; echo \'* * * * * /opt/prometheus/check_exporter/check_exporter\') | crontab -" >/dev/null 2>&1 &'
        }
